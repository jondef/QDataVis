...
QPenDialog::QPenDialog(QWidget *parent) : QDialog(parent), ui(new Ui::Dialog) {
	...
	connect(ui->pushButton_penColor, &QPushButton::clicked, this, [=]() {
		QColor color = QColorDialog::getColor();
		pen->setColor(color);
		QPalette pal = ui->pushButton_penColor->palette();
		pal.setColor(QPalette::Button, color);
		ui->pushButton_penColor->setAutoFillBackground(true);
		ui->pushButton_penColor->setPalette(pal);
		ui->pushButton_penColor->update();
		penChanged();
	});
